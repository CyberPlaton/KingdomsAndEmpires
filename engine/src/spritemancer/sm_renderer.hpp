...
namespace sm
{
	...
	namespace detail
	{
		...
		public:
			...
			spriteatlas_t get_spriteatlas(const std::string& spriteatlas_name) const;
			void unload_spriteatlas(spriteatlas_t handle);
			core::srect get_subtexture(spriteatlas_t atlas, const std::string& subtexture_name);
		...
		public:
			...
			material_t create_material(const std::string& material_name, technique_t technique,
			...
			bool create_default_material(technique_t technique,
				blending_mode mode = blending_mode_alpha, blending_equation equation = blending_equation_blend_color,
				blending_factor src = blending_factor_src_color, blending_factor dst = blending_factor_one_minus_src_alpha);
			material_t get_material(const std::string& material_name) const;
		...
		public:
			...
			texture_t get_texture(const std::string& texture_name) const;
			const raylib::Texture2D* get_gl_texture(const std::string& texture_name) const;
	...
	public:
		...
		technique_t get_technique(const std::string& technique_name);
		void unload_technique(technique_t handle);
		spriteatlas_t create_spriteatlas(const std::string& spriteatlas_name, const std::string& texture_path, const vec2_t& frame_size);
		spriteatlas_t get_spriteatlas(const std::string& spriteatlas_name) const;
		void unload_spriteatlas(spriteatlas_t handle);
		core::srect get_subtexture(spriteatlas_t atlas, const std::string& subtexture_name);
