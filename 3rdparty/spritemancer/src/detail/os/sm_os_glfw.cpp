...
namespace sm
{
	...
	{
		entry::renderer()->prepare_device();

		if (entry::renderer()->init_device(glfw_native_handle(m_mainwindow), w, h, fullscreen, vsync) != opresult_ok)
		{
			return opresult_fail;
		}
	...
	//------------------------------------------------------------------------------------------------------------------------
	float cos_glfw::frametime() const
	{
		return 0.016f;
	}
