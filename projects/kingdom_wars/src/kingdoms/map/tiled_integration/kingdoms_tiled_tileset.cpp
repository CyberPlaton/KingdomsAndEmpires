#include "kingdoms_tiled_tileset.hpp"
...
namespace kingdoms
{
	...
	//------------------------------------------------------------------------------------------------------------------------
	bool stileset_generator::load_from_file(stringview_t tilesetname)
	{
		auto result = true;

		auto* vfs = engine::cengine::service<fs::cvirtual_filesystem>();

		if (const auto info = fs::cfileinfo(tilesetname); info.exists())
		{
			if (const auto file = vfs->open(info, core::file_mode_read); file && file->opened())
			{
				...
				{
					log_critical(fmt::format("Failed to read tileset JSON data at path '{}'", info.path().data()));
					result = false;
				}
			}
			else
			{
				log_critical(fmt::format("Failed to open tileset file at path '{}'", info.path().data()));
				result = false;
			}
		}
		else
		{
			//- Report error and try to resolve the rest of the tileset to catch everything
			log_critical(fmt::format("Failed to resolve tileset file at path '{}'", info.path().data()));
			result = false;
		}
	...
	//------------------------------------------------------------------------------------------------------------------------
	bool stileset_generator::load_from_memory(const uint8_t* string, unsigned size)
	{
